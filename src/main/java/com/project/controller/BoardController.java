...
public class BoardController {
	...
	public void download(HttpServletRequest request, 
		...
		/* 브라우저는 서버로 요청을 보낼때 뷰 페이지의 인코딩 설정에 따라 데이터를
		 * 인코딩 하여 서버로 전송한다. 한글 이름을 가진 파일을 링크 형식으로 
		 * 요청하면 GET 방식 요청이 되기 때문에 톰갯 서버에서 URIEncoding
		 * 설정을  적절히 하지 않았다면 한글 이름을 가진 요청은 깨지게 된다. 
		 * 만약 뷰 페이지의 인코딩이 설정이 UTF-8이고 톰갯 서버에서 URIEncoding
		 * 설정을 하지 않았다면 브라우저에서 UTF-8로 인코딩 하여 서버로 보내고
		 * 다시 톰캣에서 ISO-8859-1로 디코딩 하여  넘겨 받기 때문에 한글은 깨지게
		 * 된다. 아래는 톰캣에서 URIEncoding 설정을 하지 않았을 경우에 ISO-8859-1로
		 * 디코딩 되어 넘어온 데이터를 바이트로 읽어서 UTF-8 문자셋으로 새로운
		 * 문자열을 생성하여 한글을 깨지지 않게 처리하는 코드 이다 톰캣 서버에서
		 * URIEncoding=utf-8로 설정했다면 아래의 변환 작업은 필요 없다. 
		 **/		
